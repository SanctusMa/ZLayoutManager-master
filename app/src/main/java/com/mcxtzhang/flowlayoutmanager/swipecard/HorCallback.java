...
public class HorCallback extends ItemTouchHelper.SimpleCallback {

    protected RecyclerView mRv;
    protected List mDatas;
    protected RecyclerView.Adapter mAdapter;

    public HorCallback(RecyclerView rv, RecyclerView.Adapter adapter, List datas) {
        this(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT,
                rv, adapter, datas);
    }

    public HorCallback(int dragDirs, int swipeDirs
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (level > 0) {
                child.setScaleX((float) (1 - HorConfig.SCALE_GAP * level + fraction * HorConfig.SCALE_GAP));

                if (level < HorConfig.MAX_SHOW_COUNT - 1) {
                    child.setScaleY((float) (1 - HorConfig.SCALE_GAP * level + fraction * HorConfig.SCALE_GAP));
                    child.setTranslationX((float) (HorConfig.TRANS_Y_GAP * level - fraction * HorConfig.TRANS_Y_GAP));
                } else {
